...
function art_censor(df::DataFrame)
    ...
    for group in grouped_df
        censored_group = group

        for i in 2:nrow(censored_group)
            if censored_group.treatment[i-1] == 1 && censored_group.treatment[i] == 0 # change i-1 to 1?
                censored_group = censored_group[1:i, :]
                censored_group[i, :censored] = 1
                #censored_group.censored[i:end] .= 1
                #first_censored = findfirst(censored_group.censored .== 1)
                #if !isnothing(first_censored)
                #    censored_group = censored_group[1:first_censored, :]
                #end
                break
            end
        end
